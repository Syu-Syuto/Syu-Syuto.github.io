...
- `写真登録.csv`: 写真のタイトル・ひとこと・撮影情報・公開ファイル名の元データ
- `templates/写真追加テンプレート.csv`: 新しい写真をまとめて登録するための入力CSV
- `outputs/photo-import-template/写真追加テンプレート.xlsx`: Excelで編集しやすい写真登録テンプレート
- `scripts/import-photos.mjs`: 写真の軽量化・EXIF削除・登録をまとめて行う処理
...
ギャラリーは `写真登録.csv` を撮影日の新しい順に表示します。通常は写真・タイトル・ひとことだけを見せ、ボタンを押すとカメラ・レンズ・焦点距離・シャッタースピード・F値・ISO感度を表示します。写真を押すと縦横比を保ったまま拡大でき、閉じるボタン・背景のクリック・Escキーで元に戻れます。空欄の情報は出しません。元のファイル名と公開用の英数字ファイル名、代替テキスト、画像サイズもCSVで管理するため、写真追加時に `src/data/photos.ts` を編集する必要はありません。
...
1. `outputs/photo-import-template/写真追加テンプレート.xlsx` を開き、「写真追加」シートへ記入します。`元ファイル名`・`タイトル`・`撮影日`・`代替テキスト`は必須です。
2. 「CSV UTF-8（コンマ区切り）」で `photo-import/写真追加.csv` として保存します。
3. 元のJPEGを `photo-import/source/` に置きます。CSVの `元ファイル名` と完全に同じ名前にします。
4. `pnpm photos:check` で、見出し・必須項目・日付・重複・画像形式を検査します。
5. 問題がなければ `pnpm photos:import` を実行します。

登録処理は、公開用ファイル名を撮影日から自動生成し、一覧用WebPと拡大用JPEGを作ります。通常のJPEGは再圧縮せずにEXIFなどのメタデータだけを削除します。向き情報に依存するJPEGだけは、正しい向きへ直すため高画質設定で再生成します。すべての写真を検査してから出力するため、途中の写真だけがCSVへ登録されることはありません。元写真と入力CSVは `photo-import/` に置くためGitには追加されません。
